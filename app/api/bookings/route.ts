...
export async function POST(req: NextRequest) {
  // Auth mode: identity check always comes first — an unauthenticated caller must see 401
  // regardless of any other internal state, never learn whether booking is under maintenance.
  // Identity comes from the Google session, never from the request body — a user can only
  // ever book/release/clear as themselves.
  if (AUTH_ENABLED) {
  ...
  // Legacy mode (AUTH_ENABLED off): original behavior — identity trusted from the client, no
  // session to check first, so the maintenance flag is the only gate.
  if (!BOOKING_ENABLED) {
    return NextResponse.json({ ok: false, error: "booking_disabled" }, { status: 503 });
  }
